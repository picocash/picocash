...
**Status: v0.1-draft, implemented** in [picocash/picocash-contracts](https://github.com/picocash/picocash-contracts) (`PicocashVault.sol`), built after the mint ran against a fake vault so the interface was dictated by a running consumer. Current Tempo **Moderato testnet** deployments: factory `0x54b7b22c660Bd2e32c0403A3F3D2A454f92DD192`, dev-mint vault `0x91Dd6be2EF2e363b088b2EEf5a20d32f10b455be` (deployed through the factory; token pathUSD `0x20c0…0000`, 2-day rotation timelock).

**Factory & on-chain discovery.** Vaults are deployed via `PicocashVaultFactory.deployVault(token, operator, timelock, name, mintUrl)` — permissionless, retaining no authority over deployed vaults (`isVault()` proves canonical bytecode; `VaultDeployed` events enumerate the ecosystem). Each vault exposes a read-only `info()`: the stable subset of the mint's `/v1/info` (name, mint API URL, token, operator, active keyset) plus live custody figures (backing balance, last published outstanding supply + timestamp) — a client can go from factory to vault to mint URL to keys with no off-chain registry, judging solvency on the way. The operator maintains the metadata via `setMintInfo` / `setActiveKeyset`.
