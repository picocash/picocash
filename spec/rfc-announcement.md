...
- **Vault contract** deployed to Moderato ([picocash-contracts](https://github.com/picocash/picocash-contracts)): memo-bound deposits, never-pausable withdrawals, one payout per melt id enforced on-chain, timelocked operator rotation.
...
5. Anything that smells unsound. The security model is three sentences: the mint's spent-secret ledger is insert-before-sign in one transaction; every signature carries a DLEQ proof; the vault can't pause withdrawals. Break any of them and we want to know before this touches real money.
