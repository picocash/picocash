...
| Operator liveness | The vault cannot be told to stop paying out, but it also cannot pay out without the operator's signature. | An abandoned mint strands holders; the publication policy makes abandonment visible (deposits halt), it does not cure it. | **Designed + prototyped**: PIP-04 §Emergency redemption — after a grace period past an overdue attestation, holders redeem at the vault directly; the vault verifies each token's DLEQ on-chain with the mint's public key (`picocash-contracts/src/emergency/`, ≈1.75 M gas/proof, verified against reference vectors). Ships in the next vault version. |
