...
| Vault deployment | The factory does not enforce a minimum rotation/fee-increase timelock or a minimum emergency grace; a vault can be deployed with `rotationTimelock = 0` or `emergencyGraceBlocks = 0` (exit disabled). | A user must read `info()` before trusting a vault — the factory's `isVault` proves bytecode, not safe parameters. | Add `MIN_TIMELOCK` to the factory in the next contract version (the current testnet vaults use 2 days); wallets/SDK will warn on short timelocks meanwhile. |
...
| Operator liveness | **Resolved in vault v2, carried into the current v3** (2026-08-21): if the attestation is overdue past a deploy-time grace period, holders redeem at the vault directly (`emergencyRedeem`, PIP-04) — the vault verifies each token's DLEQ and P2PK lock on-chain with the mint's public key. Exercised end-to-end on Moderato. | Remaining exposure: tokens spent at the mint *before* it died are indistinguishable on-chain and compete for the capped pool (cap = last attested outstanding). | Orderly-shutdown spent-set Merkle commitment (PIP-04 open item); short attestation intervals. |
