...
1. **Spec review** — read [`spec/`](spec/), especially the [MPP method draft](spec/04-mpp-method.md), and open an issue for anything unsound, underspecified, or gratuitously incompatible with prior art (Cashu NUTs, MPP methods).
2. **Test-vector cross-checks** — implement [`spec/vectors/`](spec/vectors/) in another language and report mismatches. Vector bugs are spec bugs.
